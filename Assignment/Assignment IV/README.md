...
4. I tried the Division Method first because it is the simplest, but in sequential input data, often leading to clustering. However,the Universal Hashing strategy uses a two-layer randomization process (((ax + b) mod p) mod m) to  break the patterns generated by sequential inputs, offering superior collision resistance compared to the simple Division Method.For example,it improve the condition when the input follows a pattern, the output retains that pattern,and if the input have the same mantissa, Division Method gives exactly the same Index.
